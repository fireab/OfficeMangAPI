...
export class StatisticsController {
  static findInGroup(request: Request, response: Response, next: Function) {
  ...
  static async total(request: Request, response: Response, next: Function) {
    try {
      ...
      let totalRespondedComplain: number = await Compliment.count({
        where: { year: year, responded: true },
      });
  ...
  static async customerRateTheBranch(
    ...
    try {
      let year: number = request.body.number
        ? request.body.year
        : new Date().getFullYear();
    ...
    try {
      ...
      function transformEmployees(employees: any[]) {
        return employees.map((emp) => {
          ...
          if (
            emp.employee.position?.parent_id &&
            emp.employee.position.parent_id !== 0
          ) {
            const parentEmp = employees.find(
              (e) => e.employee.position_id === emp.employee.position.parent_id
            );
          ...
          let dd = {
            ...
            averageScore: emp.averageScore,
            employee: {
              id: emp.employee.id,
              amharic_name: emp.employee.amharic_name,
              oromic_name: emp.employee.oromic_name,
              english_name: emp.employee.english_name,
              oromic_position: emp.employee.position?.name_or || "",
              position: emp.employee.position?.name_am || "",
      ...
      //   console.log("dddatatatat", JSON.stringify(data));

      // change to to proper json the result first
      let res = data.map((item: Employee) => {
  ...
  static async exportEmployeeRanks(
    ...
    const jsonData = data.map((rate: any) => ({
      ...
      "ነጥብ በ %": validateNumericAndReturnPercentile(
        rate.dataValues.averageScore
      ),
    }));
    ...
    response.setHeader(
      "Content-Disposition",
      'attachment; filename="rates.xlsx"'
    );
    response.setHeader(
      "Content-Type",
  ...
  static async findTopRatedEmployeesInMonth(
    ...
    const last_month =
      current_ethioipian_month == 0 ? 0 : current_ethioipian_month - 1;
